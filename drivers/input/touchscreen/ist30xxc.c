						   ...
						   struct ist30xx_platform_data *pdata)
{
	...
	pdata->reset_gpio = of_get_named_gpio_flags(np, "ist30xx,reset-gpio", 0, &pdata->reset_gpio_flags);
	if (pdata->reset_gpio < 0)
		return pdata->reset_gpio;

	pdata->irq_gpio = of_get_named_gpio_flags(np, "ist30xx,irq-gpio", 0, &pdata->irq_gpio_flags);
	if (pdata->irq_gpio < 0)
