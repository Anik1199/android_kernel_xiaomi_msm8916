						   ...
						   struct ft5x06_ts_platform_data *pdata)
{
	...
	/* reset, irq gpio info */
	pdata->reset_gpio = of_get_named_gpio_flags(np, "ftech,reset-gpio", 0, &pdata->reset_gpio_flags);
	...
	if (prop) {
		...
		rc = of_property_read_u32_array(np, "ftech,button-map", button_map, num_buttons);
		if (rc) {
